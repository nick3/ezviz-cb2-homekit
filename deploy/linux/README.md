...
当前地址，并在缺少所需 SAN 时自动重签；此时必须按日志重新核对新指纹。
私钥不得复制给其他主机。
...
运行也可升级，不依赖旧 `.env` 是否记录了当时的数字 ID。旧登录命令曾在
保存令牌前验证 `.env` 中的完整序列号，因此正常升级可以保留会话绑定，无需
重新配对或重新登录。已有命名卷状态绝不会被旧目录覆盖，原 `data/` 也不会
被修改。确认“家庭”画面和 Web 向导状态正常后，再把两处状态一起纳入密钥级
备份，并安全清理不再需要的旧副本。
