...
预置为另一组数字 UID/GID 时才可改这两个值；属主不匹配时容器会拒绝启动，
并在日志中显示卷属主、权限与当前进程 UID/GID，避免以不安全权限继续运行。
Compose 还会把同目录的 `./data` 只读挂载到兼容路径；全新部署可能因此看到
一个空的宿主机 `data/` 目录，实际新状态仍只写入 Docker 命名卷。
...
替换 Compose 文件后直接运行 `docker compose up -d`。启动入口会先从只读的
旧目录把 HomeKit 身份、配对列表和萤石会话自动复制到空命名卷；旧登录命令
曾在保存令牌前验证 `.env` 中的完整序列号，因此正常升级可以保留会话绑定，
无需重新配对或重新登录。已有命名卷状态绝不会被旧目录覆盖，原 `data/` 也
不会被修改。确认“家庭”画面和 Web 向导状态正常后，再把两处状态一起纳入
密钥级备份，并安全清理不再需要的旧副本。

迁移会拒绝符号链接、权限不是 `0600` 的令牌以及损坏状态，并在生成任何新
HomeKit 身份前停止容器；意外中断的复制会在下次启动时安全续传。遇到错误先查看
`docker compose logs bridge`，备份旧目录后修正提示的问题再重启；不要直接
删除旧状态。若旧部署使用了自定义 UID/GID，还要保留原 `.env` 中的
`PUID`/`PGID`，确保容器能够读取旧目录。
...
已有状态时它会拒绝覆盖。旧令牌无法可靠证明自己属于哪个序列号，因此导入
后会明确标记为“未绑定”，不会据此启动桥接；打开 Web 向导重新登录一次萤石
后才会把新会话绑定到当前摄像头。HomeKit 配对关系不受这次重新登录影响。
